...
def assess_portfolio(sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 1, 1),
    ...
    # Normalizing prices to first day
    prices = prices/prices.iloc[0]

    # Multiply each column by the allocation to the corresponding equity.
    prices_alloc = prices.multiply(allocs, axis=1)

    # Multiply these normalized allocations by starting value of overall portfolio, to get position values.
    prices_sv = prices_alloc.multiply(sv, axis=1)

    # Get daily portfolio value
    # add code here to compute daily portfolio values. Sum each row (i.e. all position values for each day)
    port_val = prices_sv.sum(axis=1)
    # print(port_val)

    # Get portfolio statistics (note: std_daily_ret means volatility)

    # ADD CODE HERE TO COMPUTE STATISTICS
    ...
    # sharpe ratio for annual standard deviation
